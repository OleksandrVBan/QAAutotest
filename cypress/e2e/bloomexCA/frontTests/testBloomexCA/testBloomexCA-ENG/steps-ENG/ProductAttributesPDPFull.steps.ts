...
class ProductAttributesPDP {
        ...
        HomePage_PO.getContainerPDPProducts().each((element,index) => {
            cy.wrap(element).within(() => {
                ...
                if (selector === 'h1') {
                    ...
                    cy.get(selector).should('have.text', 'Single Rose');
                    ...
                    cy.get(selector).should('contain','LF03-20')
                    ...
                    cy.get(selector).should('contain','$14.99')
                    cy.get(selector).should('have.css','font-family').and('eq', 'roboto, sans-serif');
                    ...
                    cy.get(selector).should('contain','$9.99')
                    cy.get(selector).should('have.css','font-family').and('eq', 'roboto, sans-serif');
                    ...
                    cy.get(selector).should('contain','$5.00')
                    cy.get(selector).should('have.css','font-family').and('eq', 'roboto, sans-serif');
                    cy.get(selector).should('have.css', 'font-size').and('eq', '16px');
                    cy.get(selector).should('have.css', 'color').and('eq', 'rgb(214, 13, 30)');
                }
                    ...
                    cy.get(selector).should('contain',"Add a single romantic red rose, complimented with lovely baby's breath and fresh greens to your order today.")
                    cy.get(selector).should('have.css','font-family').and('eq', 'roboto, sans-serif');
    ...
    checkProductsAttributesOnLandingPage() {
        ...
        ProductPage_PO.getButtonAddToCart().should('be.visible').should('not.have.attr', 'disabled')
